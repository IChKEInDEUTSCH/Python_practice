...
import re
from classes import FRG,lazyGR
...
def submit(event):
    ...
    # A = np.pi*r**2
    # air_c = 0.5*Cd*rho*A/m
    FRG.speed = 60. if (TB_spe.text == "") else float(TB_spe.text)
    FRG.height = 0. if (TB_hei.text == "") else float(TB_hei.text)
    MaxRD = FRG.AccurateAngle(lazyGR(0),lazyGR(90),0)
    # axmax = 0.0
    MaxR = FRG.getRange(lazyGR.deg,0.01)
    line.set_xdata(MaxR[0])
    line.set_ydata(MaxR[1])
    # if(x2[-1]>axmax):
    #     axmax = x2
    #     admax = degree
    # degree+=0.00001
